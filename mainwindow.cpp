...
void MainWindow::on_spinBox_catgory_valueChanged(const QString &arg1)
{
    ...
    curr_picname = u8"D:\\钢板缺陷\\"+arg1+"\\"+ QString::number( ui->spinBox_picno->value(), 10)+".bmp";//得到用户选择的文件名
    mat_opened=imread(curr_picname.toLocal8Bit().constData());  
    if(mat_opened.empty())
...
{
    curr_picname = u8"D:\\钢板缺陷\\"+ QString::number( ui->spinBox_catgory->value(), 10)+"\\"+ arg1 +".bmp";//得到用户选择的文件名
    mat_opened=imread(curr_picname.toLocal8Bit().constData());
    if(mat_opened.empty())
        cout<<"empty:"<<curr_picname.toLocal8Bit().constData()<<endl;
    else
